...
type Inventaris = {
  ...
  penanggungjawab: string   // pakai NAMA admin, bukan ID
  status: string
  kategori: string
  gedung: string
  ruangan: string
...
const PJ_OPTIONS = ['Pak Aji', 'Mas Adam', 'Mas Fikri','Mas Mono'] // 👈 list penanggung jawab manual
...
export default function KelolaInventarisPage() {
  ...
  const [form, setForm] = useState<Omit<Inventaris, 'id_fasilitas'>>({
    ...
    jumlah_unit: 0,
    jumlah_tersedia: 0,
    penanggungjawab: PJ_OPTIONS[0], // 👈 default nama admin pertama
    status: 'Tersedia',
    kategori: '',
    gedung: '',
    ruangan: '',
  ...
  const resetForm = () => {
    ...
    setForm({
      ...
      jumlah_unit: 0,
      jumlah_tersedia: 0,
      penanggungjawab: PJ_OPTIONS[0], // 👈 selalu ada
      status: 'Tersedia',
      kategori: '',
      gedung: '',
      ruangan: '',
