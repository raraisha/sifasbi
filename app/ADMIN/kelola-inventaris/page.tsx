...
export default function KelolaInventarisPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log('Users API:', jsonUsers)
  ...
  const openEdit = (row: Inventaris) => { setEditing(row); setForm({...row}); setOpen(true) }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const unit = Number(form.jumlah_unit)
    const tersedia = Number(form.jumlah_tersedia)
    if (tersedia > unit) { alert('Jumlah tersedia tidak boleh lebih besar daripada jumlah unit'); return }
    ...
    try {
      ...
      await fetchData()
      resetForm()
    } catch (e) { console.error(e); alert('Gagal simpan data, cek console') }
  }
              ...
              {!loading && filtered.map((row) => (
                <tr key={row.id_fasilitas} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 flex gap-2">
                    <button onClick={() => openEdit(row)} className="px-3 py-1 rounded bg-amber-400 text-white">Edit</button>
                    <button onClick={() => handleDelete(row.id_fasilitas)} className="px-3 py-1 rounded bg-red-500 text-white">Delete</button>
                  </td>
                </tr>
              ))}
          ...
          <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/40">
            <div className="bg-white rounded-lg p-6 max-w-2xl w-full shadow-xl">
              ...
              <div className="flex justify-end gap-2 mt-6">
                ...
                <button type="submit" onClick={handleSubmit} className="px-6 py-2 bg-gradient-to-r from-purple-500 to-blue-600 text-white rounded-md">{editing ? 'Simpan' : 'Tambah'}</button>
              </div>
            </div>
          </div>
        )}
...
function Input({label,type='text',value,onChange}:{label:string,type?:'text'|'number',value:string,onChange:(v:string)=>void}) {
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e=>onChange(e.target.value)} className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-400"/>
    </div>
  )
}
...
function Select({label,value,options,onChange}:{label:string,value:string,options:string[],onChange:(v:string)=>void}) {
  return (
    <div>
      <label className="block text-sm mb-1">{label}</label>
      <select value={value} onChange={e=>onChange(e.target.value)} className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-400">
